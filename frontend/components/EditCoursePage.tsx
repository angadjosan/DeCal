...
export function EditCoursePage({ session }: EditCoursePageProps) {
  ...
  const [existingCpfUrl, setExistingCpfUrl] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(`/api/courseForEdit/${id}`, {
          headers: {
        ...
        if (result.success && result.course) {
          ...
          setFormData({
            ...
            application_due_date: course.application_due_date 
              ? new Date(course.application_due_date).toISOString().split('T')[0]
              : "",
            cpf_file: null,
            syllabus_file: null,
          });
          ...
          setExistingCpfUrl(course.cpf || null);
          setExistingSyllabusUrl(course.syllabus_url || null);

          // Populate sections
          if (course.sections && course.sections.length > 0) {
            setSections(course.sections.map((section: any) => ({
              id: section.id || Date.now(),
              enrollmentStatus: section.enrollment_status || "Open",
              day: section.day || "",
              time: section.time || "",
              room: section.room || "",
              notes: section.notes || "",
          ...
          if (course.facilitators && course.facilitators.length > 0) {
            setFacilitators(course.facilitators.map((facilitator: any) => ({
              id: facilitator.id || Date.now(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add files only if they were changed
      if (formData.cpf_file) {
      ...
      if (!token) {
        throw new Error('You must be logged in to edit a course');
      }
      ...
      // Navigate back or to admin dashboard if admin
      ...
      <div className="max-w-[800px] mx-auto px-6 py-12">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Update your course information below. Leave file uploads empty to keep existing files.
        ...
        <Card className="p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-6">
              <div>
                <h3 className="text-[#003262] mb-4">
                  Course Materials (Optional - leave empty to keep existing files)
                </h3>
              </div>
              ...
              <div>
                ...
                {existingCpfUrl && !formData.cpf_file && (
              ...
              <div>
                ...
                {existingSyllabusUrl && !formData.syllabus_file && (
                  <p className="text-sm text-gray-600 mt-1 mb-2">
                    Current file will be kept. Upload a new file to replace it.
                  </p>
                )}
                  ...
                  >
                    {formData.syllabus_file ? (
                      ...
                      <>
                        ...
                        <p className="text-sm text-gray-600">
                          Click to upload Syllabus PDF (optional)
                        </p>
                        <p className="text-xs text-gray-400 mt-1">
                          PDF, max 50MB
                        </p>
